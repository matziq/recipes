...
ICONS = {
    "Appetizers": "≡ƒÑƒ", "Baby Food": "≡ƒì╝", "Beef": "≡ƒÑ⌐", "Breads": "≡ƒì₧",
    "Breakfast": "≡ƒì│", "Chicken": "≡ƒìù", "Desserts": "≡ƒì░", "Drinks": "≡ƒÑñ",
    "Fruit": "≡ƒìô", "Luau": "≡ƒî║", "Pasta": "≡ƒì¥", "Pork": "≡ƒÑô",
    "Salads": "≡ƒÑù", "Sandwiches": "≡ƒÑ¬", "Sauces  & Dips": "≡ƒÑ½",
    "Seafood": "≡ƒªÉ", "Sides": "≡ƒìÜ", "Snacks": "≡ƒì┐", "Soups": "≡ƒì▓",
    "Turkey": "≡ƒªâ", "Vegetarian (Tofu)": "≡ƒÑª",
    "Cakes": "≡ƒÄé", "Candies": "≡ƒì¼", "Cookies": "≡ƒì¬", "Pies": "≡ƒÑº",
    "Other": "≡ƒôä",
}
...
def _smart_pick(files: list[Path], log: list[str]) -> list[Path]:
    ...
    if with_images and without_images:
        ...
        # to preserve embedded images ΓÇö but if docx also has images, it wins.
...
<title>Recipes</title>
<link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='80' font-size='80'>≡ƒì┤</text></svg>">
<style>{BASE_CSS}</style>
</head><body>
<header>
  <h1><a href="./">≡ƒì┤ Recipes</a></h1>
...
<footer>Built from OneDrive/Recipes ΓÇó {total} recipes</footer>
<script>
let RECIPES = [];
...
def render_recipe_page(title: str, category: str, sub: str, body: str) -> str:
    ...
    root = "../" * depth
    crumbs = f'<a href="{root}index.html">Recipes</a> ΓÇ║ <a href="{root}index.html#cat-{slugify(category)}">{escape(category)}</a>'
    if sub:
        crumbs += f' ΓÇ║ {escape(sub)}'
    return f"""<!doctype html>
<html lang="en"><head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>{escape(title)} ΓÇó Recipes</title>
<link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='80' font-size='80'>≡ƒì┤</text></svg>">
<style>{BASE_CSS}</style>
</head><body>
<header><h1><a href="{root}index.html">≡ƒì┤ Recipes</a></h1></header>
<main class="container">
  <div class="crumbs">{crumbs}</div>
  <h1 style="color:var(--accent);margin-top:0">{escape(title)}</h1>
  <article class="recipe-content">{body}</article>
</main>
<footer><a href="{root}index.html">ΓåÉ Back to all recipes</a></footer>
